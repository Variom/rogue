import pygame, sys, random, time, os, math, collections
...
class Map:
    ...
    def __init__(self, nb_rooms_possible, size_possible, nb_equipment_possible, nb_creature_possible):
        ...
        # Player
        self.player = Player((offset + dpi * self._rooms[0].center().x, dpi * self._rooms[0].center().y), 40/self._size, 1000, 1000, 0.2, 0, 10, 10, 10, False)
        ...
        for i in range(len(self._mat)):
            for k in range(len(self._mat[i])):
                ...
                if self.get(Coord(k,i)) == self.empty:
                    wall = Wall((offset + dpi * k, dpi * i), self.check_down(k, i))
    ...
    def spawn_random_creature(self, pos):
        ...
        monsters = [[["dragon_front.png","dragon_back.png","dragon_right.png","dragon_left.png"],"distance", dpi, 20, 20, 5, 5, 1], [["dark_front.png","dark_back.png","dark_right.png","dark_left.png"],"cac", dpi, 15, 15, 3, 3, 2], [["piege.png", "piege.png", "piege.png", "piege.png"], "piege", dpi, 0.1, 0.1, 5, 0, 0]]#["dark.png",dpi , 20, 20, 0.2, 2]] # Image, Taille, Vie, Force
        spec = random.choice(monsters)
        creature = Creature(pos, spec[0], spec[1], spec[2], spec[3], spec[4], spec[5], spec[6], spec[7])
        self.all_creatures.add(creature)
        spec = []

    def spawn_random_equipment(self, pos):
        equipment = [["sword", "sword.png","sword.png",dpi, 10, 3, 0], ["potion", "potion_vie", "potion_vie.png",dpi, 0, 0, 20], ["potion", "potion_magie", "potion_magie.png", dpi, 0, 0, 5], ["shuriken", "shuriken.png", "shuriken.png", dpi, 1, 15, 0], ["arrow", "arc.png", "arc.png", dpi, 3, 5, 0], ["armour", "armure.png", "armor.png", dpi, 15, 0, 10 ], ["amulet", "amulet_bronze.png", "amulet_bronze.png",dpi, 0, 0, 5]]
        spec = random.choice(equipment)
        equipment = Equipment(pos, spec[0], spec[1], spec[2], spec[3], spec[4], spec[5], spec[6])
    ...
    def new_stage(self):
        ...
        self.player.resize()
        self.player.rect.x, self.player.rect.y = offset + dpi * self._rooms[0].center().x, dpi * self._rooms[0].center().y
        self.nb_creature = random.choice(self.nb_creature_possible)
        self.nb_equipment = random.choice(self.nb_equipment_possible)
        for i in range(self.nb_creature):
            self.spawn_random_creature(self.random_coord_creature())
    ...
    def new_interface(self):
        ...
        self.right_interface = Interface((offset + self._size * dpi,0), image = "right_interface.png")
    ...
    def start(self):
        ...
        self.player = Player((offset + dpi * self._rooms[0].center().x, dpi * self._rooms[0].center().y), 40/self._size, 50, 50, 3, 0, 10, 10, 10, False)
        self.player_group.add(self.player)
        self.new_stage()
...
class Wall(pygame.sprite.Sprite):
    def __init__(self ,pos, filled = True):
        ...
        self.rect = self.image.get_rect(center = (pos[0] + dpi/2, pos[1] + dpi/2))
                    ...
                    if M.player.projectile_cooldown < Map.timer:   
                        if M.player.rect.x == self.rect.x:
                            if abs(M.player.rect.y - self.rect.y) >= 3*dpi :
                                ...
                                if shot == 0:
                                    M.player.launch_projectile(self, "fireball", 10, False)
                        ...
                        if M.player.rect.y == self.rect.y:
                            if abs(M.player.rect.x - self.rect.x) >= 3*dpi :     
                                if M.player.rect.x < self.rect.x:
                                    ...
                                    centre_creature_y = M.player.rect.center[1]
                                    hauteur = dpi/2
                                    weight = self.rect.center[0] - M.player.rect.center[0]
                                elif M.player.rect.x > self.rect.x:
                                    centre_creature_x = self.rect.center[0]
                                    centre_creature_y = self.rect.center[1]
                                    hauteur = dpi/2
                                    weight = M.player.rect.center[0] - self.rect.center[0]
                                for mur in M.wall_group:
                                    if pygame.Rect(mur).colliderect((centre_creature_x,  centre_creature_y),(weight, hauteur)):
                                        shot = 1
                                M.all_creatures.remove(self)
...
class Player(pygame.sprite.Sprite):
            ...
            if (Map.timer - self.projectile_cooldown) > 1:
                if self.weapon != []:
                    ...
                    if self.weapon[0].type == "shuriken" :
                        self.launch_projectile(self, "shuriken", self.weapon[0].strenght, True)
                        self.weapon.remove(self.weapon[0])
                    elif self.weapon[0].type == "sword":
                        self.launch_projectile(self, "sword", self.weapon[0].strenght, True)
                        self.strenght -= self.weapon[0].strenght
                        self.weapon.remove(self.weapon[0])
                    elif self.weapon[0].type == "arrow":
                        self.launch_projectile(self, "arrow", self.weapon[0].strenght, False)
        ...
        if keys[pygame.K_b]:
            if (Map.timer - self.cooldown_magie) > 1:
                if self.pa_magie >= 5:
                    ...
                    l=[]
                    self.rect.x, self.rect.y = offset + dpi * random_room.center().x, dpi * random_room.center().y
    ...
    def move(self):
        scaled_direction = tuple([round(c * self.speed) for c in self.direction])
...
class Projectile(pygame.sprite.Sprite):
    ...
    def __init__(self, lanceur, type, damage, rotation = False):
        ...
        # Type of projectiles
        self.fireball = ["fireball_right.png", "fireball_up.png", "fireball_left.png", "fireball_down.png"]
        self.arrow = ["arrow_right.png", "arrow_up.png", "arrow_left.png", "arrow_down.png"]
        self.shuriken = ["shuriken.png", "shuriken.png", "shuriken.png", "shuriken.png"]
        ...
        self.rect = self.image.get_rect(center = (lanceur.rect.x + dpi/3, lanceur.rect.y + dpi/3)) 
...
class Interface(pygame.sprite.Sprite):
    ...
    def left_interface(self):
        ...
        hp = font.render(str(round(M.player.hp))+"/"+str(round(M.player.hpmax)), True, (22,196,44))
        hp_rect = hp.get_rect(topleft = (width/7,height/1.44))
        magic = font.render(str(round(M.player.pa_magie))+"/"+str(round(M.player.pa_magie_max)), True, (0,110,255))
        magic_rect = magic.get_rect(topleft = (width/7,height/1.3))
        strenght = font.render(str(round(M.player.strenght)), True, (203,17,20))
        strenght_rect = strenght.get_rect(topleft = (width/7,height/1.2))
        exp = font.render(str(round(M.player.xp))+"/"+str(round(M.player.xpmax)), True, (255,216,0))
...
class Health_Bar(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.owner.hp > 0:
            if self.type == True:
                self.image = pygame.Surface(((dpi*self.owner.hp)/self.owner.hpmax,dpi/10))
                self.image.fill((111, 210, 46))
            if self.type == False:
                self.image.fill((60, 63, 60))
...
class Sound:
    def __init__(self):
        self.sounds = {'boule_feu' : pygame.mixer.Sound("son/boule_feu.ogg"), 'escalier'  : pygame.mixer.Sound("son/escalier.ogg")}
